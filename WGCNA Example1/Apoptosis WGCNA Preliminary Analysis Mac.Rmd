...
geneTraitSignificance = as.data.frame(cor(datExpr, alactate, use = "p"));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
names(geneTraitSignificance) = paste("GS.", names(alactate), sep="");
names(GSPvalue) = paste("p.GS.", names(alactate), sep="");
...
modOrder = order(-abs(cor(MEs, alactate, use = "p")));
...
geneOrder = order(geneInfo0$moduleColor, -abs(geneInfo0$GS.alactate));
geneInfo = geneInfo0[geneOrder, ]

write.csv(geneInfo, file = "Liver.geneInfo.csv")

sig.lact.genes=geneInfo[,1:5]
sig.lact.genes=sig.lact.genes[order(sig.lact.genes$p.GS.alactate, decreasing=F),]
sig.lact.genes$adjp=p.adjust(sig.lact.genes$p.GS.alactate,method="BH")
...
heatmaply(cpmdata[sig.lact.genes$genes[1:100],],scale="row",trace="none",Colv = T,Rowv=T,cexRow = .5,cexCol=.5,main="Top 100 Lactate Genes",col=hmcolors,
  scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(
    low = "royalblue4",
    mid = "white",
    high = "firebrick4",d
