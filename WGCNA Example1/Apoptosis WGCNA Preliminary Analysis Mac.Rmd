...
alactate0H = as.data.frame(datTraits$alactate..no.0H.);
names(alactate0H) = "alactate..no.0H."
...
geneTraitSignificance = as.data.frame(cor(datExpr, alactate0H, use = "p"));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
names(geneTraitSignificance) = paste("GS.", names(alactate0H), sep="");
names(GSPvalue) = paste("p.GS.", names(alactate0H), sep="");
...
modOrder = order(-abs(cor(MEs, alactate0H, use = "p")));
...
geneOrder = order(geneInfo0$moduleColor, -abs(geneInfo0$GS.alactate..no.0H.));
geneInfo = geneInfo0[geneOrder, ]

write.csv(geneInfo, file = "Liver.geneInfo.csv")

sig.lact.genes=geneInfo[,1:5]
sig.lact.genes=sig.lact.genes[order(sig.lact.genes$p.GS.alactate..no.0H., decreasing=F),]
sig.lact.genes$adjp=p.adjust(sig.lact.genes$p.GS.alactate..no.0H.,method="BH")
...
hm=heatmaply(cpmdata[sig.lact.genes$genes[1:500],],scale="row",trace="none",Colv = T,Rowv=T,cexRow = .5,cexCol=.5,main="Top 100 Lactate (no 0H) Genes",col=hmcolors, dendrogram = "column",
  scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(
    low = "royalblue4",
    mid = "white",
    high = "firebrick4",
