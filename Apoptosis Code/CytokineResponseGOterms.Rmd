...
Cytokine.gogenes=intersect(as.character(unlist(x3[genesinallchildren])),row.names(cpmdata))
Cytokine.goterms=unique(c(edges[,1],edges[,3]))
CytokineEdges=edges
...
refset=intersect(Cytokine.goterms,allsGO)
originaledges=edges
...
dotplotterms=intersect(allsGO,Cytokine.goterms)
...
num.genes$log10.numDE[num.genes$adjp>.05]=NA
num.genes$num.DEInCat=num.genes$numDEInCat
num.genes$num.DEInCat[num.genes$adjp>.05]=NA
...
dotplot=ggplot(data=num.genes, aes(x= sample, y=term, color=neg.log10.adjp,size=num.DEInCat)) +
  geom_point(alpha = 0.9) + 
  scale_color_gradient(low = "blue", high = "red3", space = "Lab", name="adj. p-value (-log10)",n.breaks=4) +
...
```{r include=F}
geneset=intersect(allsiggenes,Cytokine.gogenes)
hmtitle="Response to Cytokines"
...
Cytokine.hmd=order[unique(row.names(order)),]
...
heatmaply(Cytokine.hmd,scale="none",trace="none",Colv = F,Rowv=F,cexRow = .25,main=hmtitle,col=hmcolors,
