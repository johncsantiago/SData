...
##same data with 0H data excluded
heatmaply(hmplotdata[,names(lactatelevels)[order(lactatelevels)]],scale="row",trace="none",Colv = F,Rowv=T,cexRow = .5,cexCol=1.5,main=hmtitle,col=hmcolors, dendrogram = "none",
  scale_fill_gradient_fun = ggplot2::scale_fill_gradient2(
    low = "royalblue4",
    mid = "white",
    high = "firebrick4",
...
pca <- prcomp(t(cpmdata[con.lactgenes,]), scale.=TRUE) 
...
text2D(x=xcoords,y=ycoords,labels = pcalabels, 
       col=pcacolors,
       xlab="PC1",ylab="PC2", ylim=c(-50,50), xlim=c(-100,100))
...
pca <- prcomp(t(cpmdata[ap.lactgenes,]), scale.=TRUE) 
gr=factor(groups$Library.ID)
...
text2D(x=xcoords,y=ycoords,labels = pcalabels, 
       col=pcacolors,
       xlab="PC1",ylab="PC2", ylim=c(-50,50), xlim=c(-50,50))
