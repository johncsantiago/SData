---
title: "Pyroptosis and Necroptosis"
...
##All sub categories accepgt for apoptotic process
specificGO=c("GO:0012501")
...
ProgrammedCellDeath.gogenes=intersect(as.character(unlist(x3[genesinallchildren])),row.names(cpmdata))

ProgrammedCellDeath.goterms=unique(c(edges[,1],edges[,3]))
```
...
refset=intersect(ProgrammedCellDeath.goterms,allsGO)
originaledges=edges
...
genesinallchildren=as.character(xx[["GO:0070269"]])
Pyroptosis.gogenes=intersect(as.character(unlist(x3[genesinallchildren])),row.names(cpmdata))
Pyroptosis.symbols=bitr((Pyroptosis.gogenes),fromType = "ENSEMBL", toType = "SYMBOL",  OrgDb="org.Hs.eg.db")
row.names(Pyroptosis.symbols)=Pyroptosis.symbols[,1]
sigPyroptosis.gogenes=Pyroptosis.symbols[intersect(Pyroptosis.gogenes,allsiggenes),]

##only one gene was found sig. DE in any of the conditions for this GO term
```

```{r include=F}
##programmed necrotic cell death [GO:0097300]

specificGO=c("GO:0097300")
temp=bpchild[specificGO[1]]
edges=matrix(specificGO[1],nrow=length(temp[[1]]),ncol=4)
edges[,2]=names(temp[[1]])
edges[,3]=temp[[1]]
...
genesinallchildren=as.character(xx[[allchildren[1]]])
Necrosis.gogenes=intersect(as.character(unlist(x3[genesinallchildren])),row.names(cpmdata))

Necrosis.goterms=unique(c(edges[,1],edges[,3]))
```

```{r fig.align="center"}
geneset=intersect(allsiggenes,Necrosis.gogenes)
hmtitle="Sig. DEG in Necrosis GO terms"
